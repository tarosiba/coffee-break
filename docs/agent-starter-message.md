...
- docs/conversation-session-2026-07-31.md（直近セッションの会話まとめ）
...
- AIニュース … カーソル君編集、目安1日2〜3件（src/lib/aiNews.ts）。最新は7/31
...
## 雑談で出た文脈（コード変更なし）
- 猛暑・熊本地震・イオン熊本店事故など → 明るいAIコーナー設置のきっかけ
- Kling は Web 版と API 料金が別。Cursor から動画は MCP + API キーが必要
- 短い動画は ffmpeg でイラストを動かす方法もある（無料）
- Paragon Pioneers をプレイ中（島開発・Colonist アップグレードまで進行）
